...
public class CompSurvey extends Activity implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        CompleteSurvey_count_textView = findViewById(R.id.limit_text);
        ...
        if (CompleteSurvey_first) {
            CompleteSurvey_first = false;
            Log.e(TAG, "onOfferWallRewardStarted: " + random_points);
            if (Constant.isNetworkAvailable(activity)) {
                random_points = "";

                random_points = String.valueOf(someEarn_controller.getCompleteSurvey_reward());
    ...
    private void onInit() {
        ...
        String CompleteSurveyCount = Constant.getString(activity, Constant.CompleteSurvey_Reward_COUNT);
        if (CompleteSurveyCount.equals("0")) {
            CompleteSurveyCount = "";
            Log.e("TAG", "onInit: scratch card 0");
        }
        if (CompleteSurveyCount.equals("")) {
            Log.e("TAG", "onInit: scratch card empty vala part");
            String currentDate = new SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(new Date());
            Log.e("TAG", "onClick: Current Date" + currentDate);
            String last_date = Constant.getString(activity, Constant.LAST_DATE_CompleteSurvey_Reward);
            Log.e("TAG", "Lat date" + last_date);
            if (last_date.equals("")) {
                Log.e("TAG", "onInit: last date empty part");
                setScratchCount("10000");
                Constant.setString(activity, Constant.CompleteSurvey_Reward_COUNT, "10000");
                Constant.setString(activity, Constant.LAST_DATE_CompleteSurvey_Reward, currentDate);
            } else {
                ...
                try {
                    ...
                    if (difference_In_Days > 0) {
                        Constant.setString(activity, Constant.LAST_DATE_CompleteSurvey_Reward, currentDate);
                        Constant.setString(activity, Constant.CompleteSurvey_Reward_COUNT, "10000");
                        setScratchCount(Constant.getString(activity, Constant.CompleteSurvey_Reward_COUNT));
            ...
            setScratchCount(CompleteSurveyCount);
        }
    }

    private void setScratchCount(String string) {
        if (string != null || !string.equalsIgnoreCase(""))
            CompleteSurvey_count_textView.setText("Your daily limit left to collect this reward is = " + string);
    }
    ...
    private void showDialogPoints(final int addOrNoAddValue, final String points, final int counter) {
        ...
        if (Constant.isNetworkAvailable(activity)) {
            ...
            add_btn.setOnClickListener(view -> {
                ...
                first_time = true;
                CompleteSurvey_first = true;

                poiints = 0;
                if (addOrNoAddValue == 1) {
                    if (points.equals("0")) {
                        String current_counter = String.valueOf(counter - 1);
                        Constant.setString(activity, Constant.CompleteSurvey_Reward_COUNT, current_counter);
                        setScratchCount(Constant.getString(activity, Constant.CompleteSurvey_Reward_COUNT));
                        dialog.dismiss();
                    } else {
                        String current_counter = String.valueOf(counter - 1);
                        ...
                        try {
                            ...
                            Intent i = new Intent(CompSurvey.this, Main.class);
                            startActivity(i);
                            finish();

                        } catch (NumberFormatException ex) {
                            Log.e("TAG", "onCompleteSurvey: " + ex.getMessage());
                        }
                        dialog.dismiss();
                    }
                } else {
                ...
                if (CompleteSurvey_count == Integer.parseInt(someEarn_controller.getRewarded_and_interstitial_count())) {
                    if (rewardShow) {
                        Log.e(TAG, "onReachTarget: rewaded ads showing method");
                        showDailog();
                        rewardShow = false;
                        interstitialShow = true;
                        CompleteSurvey_count = 1;
    ...
    public void onPollfishSurveyCompleted(@NotNull SurveyInfo surveyInfo) {
        ...
        if (first_time) {
            first_time = false;
            Log.e("onCompleteSurvey", "Complete");
            Log.e("onCompleteSurvey", "Complete" + random_points);
            String count = CompleteSurvey_count_textView.getText().toString();
            String[] counteee = count.split("=", 2);
            String ran = counteee[1];
            Log.e(TAG, "onCompleteSurveyComplete: " + ran);
            int counter = Integer.parseInt(ran.trim());
            if (counter == 0) {
                showDialogPoints(0, "0", counter);
            } else {

                showDialogPoints(1, random_points, counter);
            }
        }
